...
# Matriz de criptografia (K)
# Usada para cifrar o texto através de multiplicação matricial
# | 4  3 |
# | 1  2 |
k11 = 4
k12 = 3
...
# Matriz inversa de K (K⁻¹), usada na descriptografia
# Calculada previamente com aritmética modular na base 36
# Aplicar K⁻¹ sobre o texto cifrado recupera o texto original
# | 22  21 |
# |  7   8 |
ki11 = 22
ki12 = 21
...
def converter_texto_numero(texto):
    ...
    for letra in texto:
        # Padroniza para maiúsculo, garantindo que 'a' e 'A' sejam tratados igual
        letra = letra.upper()
...
def converter_numero_texto(numeros):
    ...
    for numero in numeros:
        # O % 36 garante que o índice sempre caia dentro do conjunto válido (0 a 35)
        # Isso é necessário pois a multiplicação matricial pode gerar valores grandes
...
def criptografar_hill(texto):
    ...
    """
    # Converte o texto para uma lista de índices numéricos
    numeros = converter_texto_numero(texto)

    # A Cifra de Hill exige blocos de tamanho par (blocos de 2)
    # Se o texto tiver quantidade ímpar de caracteres, adiciona 'A' (índice 0) como padding
    if len(numeros) % 2 != 0:
        numeros.append(0)

    resultado = []
    # Processa os números em blocos de 2: [x, y], [x, y], ...
    for i in range(0, len(numeros), 2):

        x = numeros[i]
        y = numeros[i + 1]

        # Multiplicação do bloco [x, y] pela matriz K:
        # | novo1 |   | 4  3 |   | x |
        # | novo2 | = | 1  2 | × | y |  (mod 36)
        novo1 = (k11 * x + k12 * y) % 36  # linha 1 da matriz × bloco
        novo2 = (k21 * x + k22 * y) % 36  # linha 2 da matriz × bloco

        resultado.append(novo1)
        resultado.append(novo2)

    # Converte os índices cifrados de volta para texto
...
def descriptografar_hill(texto_criptografado):
    ...
    """
    # Converte o texto cifrado para uma lista de índices numéricos
    numeros = converter_texto_numero(texto_criptografado)

    resultado = []
    # Processa em blocos de 2, assim como na criptografia
    for i in range(0, len(numeros), 2):

        x = numeros[i]
        y = numeros[i + 1]

        # Multiplicação do bloco [x, y] pela matriz inversa K⁻¹:
        # | original1 |   | 22  21 |   | x |
        # | original2 | = |  7   8 | × | y |  (mod 36)
        # Isso desfaz exatamente a operação da criptografia
        original1 = (ki11 * x + ki12 * y) % 36  # linha 1 da inversa × bloco
        original2 = (ki21 * x + ki22 * y) % 36  # linha 2 da inversa × bloco

        resultado.append(original1)
        resultado.append(original2)

    # Reconstrói e retorna o texto original
    return converter_numero_texto(resultado)
